...
export class MT63Client {
  ...
  processAudio(floatArr: Float32Array, len: number, sampleRate: number = 8000) {
    // If the sample rate is already 8000 this will be a total no-op,
    // otherwise resample the audio to 8000hz
    let ReSamp = new Resampler(sampleRate, 8000, 1, floatArr);
    len = ReSamp.resampler(len);
    floatArr = ReSamp.outputBuffer;

    // Allocate memory and copy the buffer into the wasm heap
    let size = 4;
    let numBytes = len * size;
    let ptr = wasmModule.mod._malloc(numBytes);
    wasmModule.mod.HEAPF32.set(subarray(floatArr, len), ptr / size);

    // Call the Wasm function to process the audio
